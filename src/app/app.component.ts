...
//  place_name;
  ...
  let localHourlyData;  
    ...
    //  console.log(hourly_data);
      localHourlyData = hourly_data.features[0].properties.timeSeries[2];
      console.log(_lat, _lon, localHourlyData);
      

      this.hourlyData.push(localHourlyData);
      },
    )
  }
  ...
  async getDailyData(_lat, _lon) {
    ...
    //this.httpClient.get(`./hourlyData.json`, {headers:this.headers}).subscribe((daily_data:any) => {
      ...
      localDailyData = daily_data.features[0].properties.timeSeries;
      localDailyData.shift();
      console.log(_lat, _lon, localDailyData);


      this.dailyData.push(localDailyData);
  ...
  async ngOnInit() {
    for(var i = 0;i<this.place_data.length;i++) {
      await this.getData(this.place_data[i].lat, this.place_data[i].lon);
